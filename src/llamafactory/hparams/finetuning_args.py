...
class RLHFArguments:
    ...
    pref_loss: Literal["sigmoid", "hinge", "ipo", "kto_pair", "orpo", "simpo", "nca_pair", "grpo"] = field(
        default="sigmoid",
    ...
    # +++++++++++++添加新参数++++++++++++++
    # 添加推理质量评估相关参数  
    use_reasoning_quality: bool = field(  
        default=False,  
        metadata={"help": "Whether to use reasoning quality evaluation during DPO training."},  
    )  
      
    reasoning_weight: float = field(  
        default=0.3,  
        metadata={"help": "Weight to apply to reasoning quality difference in loss calculation."},  
    )  
      
    # 格式奖励参数  
    format_weight: float = field(  
        default=0.3,  
        metadata={"help": "Weight for format reward in reasoning quality evaluation."},  
    )  
      
    # 余弦奖励参数  
    cosine_weight: float = field(  
        default=0.3,  
        metadata={"help": "Weight for cosine reward in reasoning quality evaluation."},  
    )  
    cosine_min_len_value_wrong: float = field(  
        default=-0.5,  
        metadata={"help": "Minimum value for incorrect answers in cosine reward."},  
    )  
    cosine_max_len_value_wrong: float = field(  
        default=0.0,  
        metadata={"help": "Maximum value for incorrect answers in cosine reward."},  
    )  
    cosine_min_len_value_correct: float = field(  
        default=1.0,  
        metadata={"help": "Minimum value for correct answers in cosine reward."},  
    )  
    cosine_max_len_value_correct: float = field(  
        default=0.5,  
        metadata={"help": "Maximum value for correct answers in cosine reward."},  
    )  
    cosine_max_len: int = field(  
        default=2048,  
        metadata={"help": "Maximum length for cosine reward calculation."},  
    )
...
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments, BAdamArgument):
    ...
    def __post_init__(self):
        ...
        # 确保GRPO也可以使用参考模型  
        if self.stage == "dpo" and self.pref_loss == "grpo" and getattr(self, "use_reasoning_quality", False):  
